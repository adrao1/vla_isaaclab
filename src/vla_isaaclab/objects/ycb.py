...
def configure_sugar_box(scene, world) -> None:
    """Load only the task's manipulated sugar box."""
    scene.object = ycb_cfg("004_sugar_box", world.object_position, 0.088)
    scene.object.prim_path = "{ENV_REGEX_NS}/Object"


YCB_BASIC_OBJECTS = ObjectSetDefinition(
    component_id="Objects-YCB-Basic-v0",
    capabilities=frozenset({"manipulation_object", "physical_goal", "distractors", "cracker_box"}),
    entity_roles={"manipulation_object": "object", "cracker_box": "object", "goal": "goal"},
    configure_scene=configure_ycb,
    metadata={"cracker_box_dimensions_m": [0.060, 0.158, 0.214]},
)


YCB_SUGAR_BOX_OBJECTS = ObjectSetDefinition(
    component_id="Objects-YCB-SugarBox-v0",
    capabilities=frozenset({"manipulation_object", "sugar_box"}),
    entity_roles={"manipulation_object": "object", "sugar_box": "object"},
    configure_scene=configure_sugar_box,
